...
- **Khung phụ đề trong `www/index.html` bị hẹp bất thường so với video**: đã
  sửa bằng cách quy đổi `widthPercent` ra px qua `getBoundingClientRect()`
  của `#videoStage` thay vì set `width` bằng CSS `%` trực tiếp trên phần tử
  `position:absolute` (xem hàm `applyCaptionSettings()`).
