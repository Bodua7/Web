...
- **Đọc to bản dịch (TTS)**: tuỳ chọn bật/tắt, tắt tiếng nhanh (nhớ lại mức âm lượng cũ), chỉnh
  âm lượng (0-200%), tốc độ nói và tông giọng chỉnh được, có nút "Nghe thử", chọn giọng đọc (danh sách CHỈ liệt kê giọng tiếng Việt `vi`/`vi-VN`). Trong app native dùng plugin `TtsBoostPlugin`
...
## Thay đổi gần đây (2026-09-19)

- **Sửa lỗi khung video YouTube trắng tinh khi bấm "Phát"** (chỉ hiện sau khi bấm "Bắt đầu nghe").
  Gốc rễ: `#videoPlayerFrame` trong `index.html` là thẻ `<iframe>` không có `src`. `YT.Player`
  chỉ TẠO iframe và nạp `videoId` khi phần tử đích là `<div>`; nếu đích đã là `<iframe>` nó chỉ gắn
  vào iframe đó (src rỗng) rồi bỏ qua `videoId` — khung trắng mãi, không `onReady`, không `onError`.
  Đã đổi thành `<div id="videoPlayerFrame">` (kèm bước phòng thủ trong `app.js`: nếu đích vẫn là
  iframe thì tự thay bằng div trước khi tạo player). **Đã xác nhận trên máy thật**: log ghi
  `Tạo player (host=youtube.com)` → `onReady OK` sau ~1 giây → `state=1` (đang phát) ngay lần bấm
  "Phát" đầu tiên, không cần fallback; bắt audio hệ thống (`mode=system`) nhận được tiếng video và
  Whisper ra bản dịch bình thường.
- **Đã thử và loại trừ** (giữ lại code vì vô hại, xem mục YouTube bên dưới): race callback
  `onYouTubeIframeAPIReady` (`isYtApiLoaded()`/`waitForYtApi()`), ép vẽ lại iframe
  (`forceVideoRepaint()`), thông báo lỗi hiện ngay dưới ô dán link (`showVideoStatus()` — panel log
  mặc định ẩn nên trước đây lỗi vô hình).
- **Chẩn đoán player YouTube**: log `[YT]` (tạo player + `origin`, `onReady`, mỗi lần đổi
  `state`), watchdog 6 giây — chưa `onReady` thì ghi `iframe src` thật rồi thử lại 1 lần qua
  `youtube-nocookie.com`.
- **Giọng đọc chỉ còn tiếng Việt**: `renderTtsVoiceOptions()` lọc theo locale `vi`/`vi-VN`/`vi_VN`
  (`isVietnameseVoice()`); giọng đã lưu từ trước mà không phải tiếng Việt bị xoá khỏi settings khi
  mở app (nếu không, `speakTranslated()` vẫn đọc bằng giọng cũ dù danh sách không còn hiện). Máy
  chưa có giọng Việt → danh sách chỉ còn mục mặc định; cần cài dữ liệu giọng Việt trong cài đặt
  Text-to-speech của máy.
- **TTS / Boost (báo "Boost chưa hoạt động")**: nguyên nhân nghi ngờ chính là `AndroidManifest.xml`
  thiếu `<queries>` cho `android.intent.action.TTS_SERVICE` (Android 11+) → `TextToSpeech` init lỗi,
  không có giọng, không ra tiếng. `scripts/patch-manifest.py` giờ tự thêm khối này (chạy lại nhiều
  lần vẫn an toàn). `TtsBoostPlugin.kt`: init thất bại thì `getVoices()`/`speak()` reject có lý do
  (trước đây treo vô hạn), thêm `getStatus()` để chẩn đoán, đọc WAV theo chunk RIFF thật (không giả
  định header 44 byte). `app.js` ghi log `[TTS] status/getVoices` — nếu vẫn lỗi, xem các dòng này.
  **Chưa xác nhận trên máy thật.**
- **Boost 200%**: trần `MAX_VOLUME` 1.5 → 2.0, thanh trượt 0-200%. Đường >100% dùng
  `boostPcm16()` (`TtsGainUtils.kt`) — bộ giới hạn mềm (tuyến tính dưới ngưỡng 0.6, nén tanh phía
  trên) thay vì clip cứng `applyGainToPcm16()`, để 2x ít vỡ tiếng hơn; vẫn còn méo nhẹ ở đỉnh (đánh
  đổi vật lý). Có test JUnit mới trong `TtsGainUtilsTest.kt` (đã mô phỏng số bằng Python vì không
  compile Kotlin thật được).
- **Tốc độ nói / tông giọng / Nghe thử**: `TtsBoostPlugin.speak()` nhận thêm `rate` (0.5-2.0) và
  `pitch` (0.5-2.0) → `setSpeechRate()`/`setPitch()`; UI có 2 thanh trượt + nút "Nghe thử" (bỏ qua
  công tắc đọc to và tắt tiếng). Android KHÔNG cung cấp thông tin giọng nam/nữ đáng tin cậy nên
  không có bộ lọc nam/nữ — chọn giọng bằng tai ("Giọng 1, 2, …"), thanh tông giọng chỉ chỉnh thêm.
- **Chống vòng lặp phản hồi TTS → STT** (phát hiện khi audit): cấu hình bắt audio hệ thống khớp
  `USAGE_MEDIA`/`USAGE_UNKNOWN` và giọng đọc mặc định cũng là `USAGE_MEDIA` → giọng đọc dịch bị bắt
  ngược vào Whisper → dịch → đọc lại. Sửa 2 lớp: (1) `TtsBoostPlugin` phát TTS (cả `tts.speak()` lẫn
  `AudioTrack` của đường Boost) với `USAGE_ASSISTANCE_ACCESSIBILITY` — không khớp cấu hình capture
  nên không bị bắt, video YouTube trong app (`USAGE_MEDIA`) vẫn được bắt; không dùng `excludeUid`
  vì video cùng uid với app. (2) Luồng MIC (mic thu cả tiếng loa) không lọc được theo loại audio →
  `SttReadLoop` coi luồng mic là im lặng (zero mẫu, bỏ VAD) trong lúc TTS đang đọc + 400ms đuôi vang,
  dựa trên `TtsSpeakingState` (`TtsSpeakingState.kt`, có `TtsSpeakingStateTest.kt`). Đánh đổi: mất tiếng
  mic trong khoảng TTS đọc (half-duplex). Listener TTS giờ là 1 cái cố định dùng chung 2 đường.
  **Chưa xác nhận trên máy thật**: (a) giọng đọc không còn bị bắt ở chế độ system; (b) âm lượng giọng
  đọc vẫn theo âm lượng media (STREAM_ACCESSIBILITY thường là alias của media nhưng tuỳ hãng).
- **Nhịp tim phiên nghe dài (soak-test)**: khi đang nghe, mỗi 60s ghi 1 dòng log
  `[phiên] Np | heap=..MB | đoạn=.. | chờ-thử-lại=.. | audioLevel cách ..s | online=..`
  (`soakHeartbeat()` trong `app.module.js`; `window.__getPendingCount` từ `app.js`). Cảnh báo nếu
  >15s không nhận `audioLevel` từ native (luồng đọc audio đứng/bị kill). Bản ghi cuối lưu
  `localStorage` (`atun_soak_v1`); mở app lại mà phiên trước chưa "kết thúc" → log
  `Phiên nghe trước KHÔNG kết thúc bình thường` kèm số phút đã chạy. Chạy thử phiên dài phải làm
  TAY trên máy thật — không tự động hoá/chạy được ở môi trường dev (xem quy trình ngay dưới).

**Quy trình chạy thử phiên nghe 30 phút+ (làm tay trên máy thật)**: (1) tick "Hiện log kỹ thuật",
xoá log; (2) dán link video dài ≥40 phút (hoặc phát video/podcast ở app khác nếu chọn bắt audio hệ
thống), bấm "Bắt đầu nghe"; (3) tắt màn hình ~10 phút rồi mở lại, làm lại 1 lần nữa sau ~20 phút;
(4) ít nhất 1 lần tắt Wi-Fi/4G ~1 phút rồi bật lại (kiểm tra hàng đợi thử lại); (5) dừng nghe, chụp
toàn bộ các dòng `[phiên]`. Đạt khi: số đoạn tăng đều, heap không tăng vọt liên tục (ổn định hoặc
tăng chậm rồi đi ngang), `chờ-thử-lại` về 0 sau khi có mạng lại, không có cảnh báo `audioLevel`.
...
media/TTS hệ thống bị khoá cứng ở 100%. Trần khuếch đại đặt ở 200% (`TtsBoostPlugin.MAX_VOLUME`)
...
đại → `AudioTrack` phát ra tiếng thật, không câm hay lỗi; mức vỡ tiếng ở 200% có chấp nhận
...
`new YT.Player('videoPlayerFrame', {...})` **THAY HẲN** phần tử đích bằng iframe riêng của chính
nó — mọi chỗ code cũ dựa vào giữ tham chiếu/id/style inline của phần tử gốc đều phải sửa theo (đã
sửa 2 chỗ: listener `fullscreenchange` chuyển sang so sánh quan hệ cha-con với `videoStageEl` thay
vì so tham chiếu iframe cũ; CSS style/fullscreen-sizing của iframe chuyển từ selector
`#videoPlayerFrame` sang `#videoStage iframe`).

**QUAN TRỌNG — phần tử đích PHẢI là `<div>`, KHÔNG được là `<iframe>`.** Bản trước ghi nhầm là
"thay hẳn cả `<iframe>` gốc": thực tế nếu đích đã là `<iframe>`, `YT.Player` chỉ gắn vào iframe đó
và bỏ qua `videoId` (nó mong `src` đã có sẵn `enablejsapi=1`). `index.html` cũ để `<iframe
id="videoPlayerFrame">` không `src` → khung trắng tinh, không `onReady`/`onError`, log không có gì
(triệu chứng thật đã gặp; chẩn đoán được nhờ dòng `6s chưa onReady. iframe src=` — src rỗng). Giờ
`index.html` dùng `<div id="videoPlayerFrame">`, và `startYoutubePlayer()` tự đổi iframe→div nếu
gặp `index.html` cũ. Đường thử lại qua `rebuildYtPlayer()` (watchdog) luôn dùng div nên chạy đúng
ngay cả khi chưa sửa index.html — đó là lý do trước đây "thử lại bằng nocookie" nhìn như có tác
dụng, thực ra không liên quan tới nocookie.

Chẩn đoán/độ bền còn giữ trong `app.js` (không bắt buộc sau khi sửa gốc rễ, nhưng vô hại):
`isYtApiLoaded()` + `waitForYtApi()` (callback `onYouTubeIframeAPIReady` có thể bị bỏ lỡ vì
`supabase.js` nạp đồng bộ chen giữa `iframe_api` và `app.js` — hỏi thẳng `window.YT.Player` thay vì
chỉ tin cờ callback, chờ tối đa 12 giây, nạp lại script 1 lần); `showVideoStatus()` (thông báo lỗi
hiện dưới ô dán link, vì panel log mặc định ẩn); watchdog 6 giây + log `[YT]`
(tạo/`onReady`/`state`); `forceVideoRepaint()` (ép vẽ lại iframe — giả thuyết lỗi compositing
WebView đã bị loại trừ, có thể gỡ nếu muốn gọn code).
...
2/5/100/101/150 theo tài liệu YouTube, thêm 153 = lỗi cấu hình/nguồn nhúng) — trước đó video lỗi (ID sai/chặn nhúng) sẽ im lặng mãi
...
- **Video (YouTube IFrame Player API)**: đã xác nhận trên máy thật (2026-09-19) — bấm "Phát" lần
  đầu là chạy ngay (`onReady OK` → `state=1`), phát/dừng nhiều lần OK, không còn lỗi
  `ytPlayer.loadVideoById is not a function` hay khung trắng (xem mục "Âm lượng video gốc").
  Còn cần test: kéo thanh trượt âm lượng có thực sự chỉnh được tiếng phát ra không (mới xác nhận
  slider hiện đúng 100%, chưa xác nhận hiệu lực thật); bấm "Phát" đổi video MỚI trong lúc player
  trước chưa kịp sẵn sàng (nhánh `pendingVideoIdForPlayer`); logic tự chuyển hướng fullscreen
  (nút fullscreen CỦA YouTube, không phải nút "Phóng to" của app); giọng đọc chỉ-tiếng-Việt (máy
  thật có ≥1 giọng `vi-VN` hiện trong dropdown và đọc đúng).
- **TTS (`TtsBoostPlugin`)**: đã xác nhận UI hiện đúng trên máy thật (checkbox, thanh trượt, nút
  tắt tiếng, dropdown giọng đọc đều hiện, danh sách giọng đọc tải được ít nhất "(Mặc định của
  máy)"). CHƯA xác nhận (chưa có log/ảnh nào cho thấy đã thử): bấm "Bắt đầu nghe" có thực sự ĐỌC
  TO bản dịch không; đường ≤100% (`tts.speak()` thẳng) và đường >100% (`synthesizeToFile` →
  khuếch đại PCM → `AudioTrack`) đều phát ra tiếng thật hay không; kéo thanh trượt vượt 100% có
  nghe to hơn thật không (mục đích chính của cả tính năng); không bị dồn ứ hàng đọc khi nghe liên
  tục nhiều câu; nút 🔇 tắt/bật đúng; mức vỡ tiếng ở 200% có chấp nhận được không.
