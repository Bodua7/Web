...
đích (theo đúng bố cục UI yêu cầu), không phải bug.
...
- **Xuất file** (nhật ký dịch .txt/.srt, file đã dịch) dùng `@capacitor/filesystem` +
  ...
  giữ lại làm fallback khi chạy ngoài app (trình duyệt thường lúc dev/test). **Chưa xác nhận trên
  máy Android thật** — mới thêm 2 dependency, cần `npm install` + build lại 1 lần rồi thử xuất
  file thật để chắc luồng Filesystem/Share hoạt động đúng (đặc biệt là màn hình chia sẻ có mở lên
  và file có lưu được không).
...
- **Test lại bố cục `index.html` trên máy thật** sau khi gộp khối dịch văn bản/file + dán-link-
  video vào trong `#mainControlSection` (xem mục "Bố cục `index.html`" ở trên) — xác nhận: dịch
  văn bản/file và dán link video vẫn hoạt động bình thường sau khi mở rộng "Cài đặt nghe/dịch";
  và xác nhận việc giờ 2 công cụ này bị khoá tới khi xong onboarding là chấp nhận được (không
  phải quên mất use-case "dùng thử công cụ dịch trước khi cấp quyền mic/thông báo").
- **Test xuất file (.txt/.srt/nhật ký dịch/file đã dịch) trên máy Android thật** sau khi thêm
  `@capacitor/filesystem`/`@capacitor/share` — chạy `npm install` rồi build lại (2 plugin native
  mới cần `npx cap sync` để đăng ký), sau đó thử bấm từng nút xuất và xác nhận màn hình chia
  sẻ/lưu của Android có mở đúng, file có lưu được không.
