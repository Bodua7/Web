...
│   ├── ModelCatalog.kt / ModelDownloader.kt   # Catalog + hàm tải/giải nén thuần cho ModelManagerPlugin
│   ├── LidEngine.kt            # Nhận diện ngôn ngữ bằng model âm học ONNX - xem mục "Nhận diện
│   │                           # ngôn ngữ (LID)" bên dưới
│   └── AppLog.kt
...
│   ├── patch-onnx-gradle.py    # Thêm dependency ONNX Runtime (dùng cho model LID, xem dưới)
...
│   ├── youtube-extract/        # Lấy stream/info video YouTube + phụ đề có sẵn (lời bài hát)
│   └── groq-relay/             # Relay gọi Groq API + DeepL dự phòng (giấu API key khỏi client)
...
│   ├── caption_frame/          # Port tay applyCaptionFrame() (BrowserActivity.kt/CaptionRelay.kt)
│   ├── cloud_stt_segment/      # Port tay buildWavFile() + cắt đoạn Whisper Cloud (AudioCaptureService.kt)
│   ├── vosk_smart_pause/       # Port tay state machine "ngắt câu thông minh" nhánh Vosk (SttReadLoop.kt)
│   ├── adblock/                # Test AdBlockList.kt (exact-or-subdomain match)
│   └── browser_url/            # Test chuẩn hoá URL thanh địa chỉ Web mini (BrowserTabManager.kt)
├── capacitor.config.ts
├── package.json
...
cd ../caption_frame && pip install pytest && pytest -v
cd ../cloud_stt_segment && pip install pytest && pytest -v
cd ../vosk_smart_pause && pip install pytest && pytest -v
cd ../adblock && pip install pytest && pytest -v
cd ../browser_url && pip install pytest && pytest -v
```

(CI chạy cả 8 thư mục này cùng lúc bằng 1 lệnh `pytest`, xem
`.github/workflows/android-build.yml`.)

## Nhận diện ngôn ngữ (LID)

Ngoài cách phân xử "so confidence text-decode" mặc định của chế độ Tự động
nhận diện (nhiều `VoskEngine` chạy song song, xem `MultiLangVoskEngine.kt`),
app còn có 1 lớp phân xử phụ bằng model âm học thật:

- **Model**: `silero_lang_detector_95` (tác giả snakers4/silero-vad, giấy
  phép MIT), bản export ONNX (~17MB), phân loại 90 ngôn ngữ trực tiếp từ
  waveform 16kHz - không qua bước decode ra "từ" như Vosk, nên vẫn phân biệt
  tốt hơn với audio có nhạc nền to/nhiễu. Chạy bằng ONNX Runtime for Android
  (`native/LidEngine.kt`, dependency thêm qua `scripts/patch-onnx-gradle.py`).
- **minSdkVersion**: `onnxruntime-android:1.27.0` yêu cầu tối thiểu **API 24**
  (Android 7.0 Nougat) - `patch-onnx-gradle.py` tự nâng `minSdkVersion` trong
  `android/variables.gradle` lên 24 nếu đang thấp hơn (mặc định Capacitor sinh
  ra là 22). Nghĩa là app **không cài được** trên máy Android 6.0 (Marshmallow)
  trở xuống hoặc cũ hơn - đánh đổi để có tính năng nhận diện ngôn ngữ bằng
  model ONNX. Nếu cần hỗ trợ máy cũ hơn API 24, phải bỏ hẳn dependency này
  (mất tính năng LID bằng model ONNX, quay về cách so confidence Vosk cũ).
- **Khi nào dùng tới**: CHỈ khi 2+ recognizer Vosk chốt câu cùng lúc với
  confidence gần bằng nhau (xem `TIE_CONFIDENCE_DELTA` trong
  `MultiLangVoskEngine.kt`) - không chạy cho mọi câu, chỉ "phân xử" lúc Vosk
  mù mờ.
- **Tải model**: riêng lẻ, không nằm trong danh sách ngôn ngữ STT ở mục
  "Ngôn ngữ hỗ trợ" bên dưới (`ModelManagerPlugin.downloadLidModel()`/
  `isLidModelDownloaded()`, lưu vào `modelsDir/_lid/`). Nếu chưa tải,
  app tự rơi về cách phân xử so confidence cũ, không chặn tính năng Tự động
  nhận diện.
- **An toàn**: chưa kiểm nghiệm thực tế trên thiết bị Android thật (chưa có
  máy để build-test - xem ghi chú tương tự ở `VadDetector.kt`). Toàn bộ hàm
  trong `LidEngine.kt` tự bọc try/catch, trả về null/false khi lỗi thay vì
  throw, nên 1 lỗi ONNX Runtime (thiếu native lib, model hỏng...) không làm
  crash phiên nghe hay chặn luồng Vosk chính.
...
`groq-relay` cần biến môi trường `GROQ_API_KEY` (bắt buộc, dịch LLM +
transcribe), `GROQ_API_KEY_2` (tuỳ chọn - key Groq thứ 2, tự động thử khi key
1 bị 401/429 do hết quota), `GEMINI_API_KEY` (tuỳ chọn — nếu không set, app tự
bỏ qua tầng Gemini, lấy key miễn phí tại Google AI Studio:
https://aistudio.google.com/apikey), `DEEPL_API_KEY` (tuỳ chọn — nếu không
set, app tự bỏ qua tầng DeepL) và `AZURE_TRANSLATOR_KEY` +
`AZURE_TRANSLATOR_REGION` (tuỳ chọn — nếu không set, app tự bỏ qua tầng Azure
Translator, xem mục "Chuỗi dịch" bên dưới) được set trong Supabase project
settings (Edge Function secrets) — không hard-code trong code. Key DeepL Free
API có hậu tố `:fx` (server tự nhận diện để gọi đúng `api-free.deepl.com`
thay vì `api.deepl.com`). `AZURE_TRANSLATOR_REGION` là vùng Azure nơi tạo
resource Translator (vd `southeastasia`) - bắt buộc phải đúng khớp, sai vùng
Azure trả lỗi xác thực khó hiểu chứ không phải lỗi "sai vùng" rõ ràng.
`AZURE_TRANSLATOR_ENDPOINT` (tuỳ chọn, mặc định
`https://api.cognitive.microsofttranslator.com`) chỉ cần đổi nếu dùng
resource loại "Custom domain".

## Bảng theo dõi hết quota theo THÁNG (DeepL / Azure)

Chạy 1 lần trong Supabase Studio → **SQL Editor**, dán nội dung file
`supabase-functions/groq-relay/migration_translate_quota_status.sql` rồi
**Run**. Bảng `translate_quota_status` lưu thời điểm + tháng mà DeepL (HTTP
456 "Quota Exceeded") hoặc Azure (HTTP 403 hoặc body chứa "quota") được xác
nhận hết hạn mức free-tier trong tháng đó — Edge Function tự kiểm tra bảng
này trước mỗi lần gọi DeepL/Azure để bỏ qua thẳng nếu tháng hiện tại trùng
tháng đã hết quota, và tự "mở lại" khi sang tháng mới (chỉ cần so sánh chuỗi
`YYYY-MM`, không cần job/cron dọn dẹp). Đọc/ghi bảng qua service role key
(`SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`) — 2 biến này Supabase **tự
động** cấp sẵn cho mọi Edge Function, không cần set thêm secret. Xem trạng
thái hiện tại qua `?op=debug` (trường `monthlyQuotaStatus`).

Không áp dụng cơ chế này cho Groq/Gemini vì free-tier 2 dịch vụ đó giới hạn
theo phút/ngày (không phải theo tháng) — 2 dịch vụ này dùng circuit breaker
ngắn hạn riêng (xem mục "Chuỗi dịch" bên dưới).

## Chuỗi dịch (fallback 6 tầng)

`translateText()` trong `www/app.js` thử lần lượt 6 nguồn dịch, mỗi nguồn lỗi
thì rơi xuống nguồn tiếp theo (không có tầng nào chặn cứng cả luồng nghe):

| # | Nguồn | Kiểu | Phủ ngôn ngữ | Ghi chú |
|---|---|---|---|---|
| 1 | ML Kit Translate | Offline, không giới hạn | 24/29 mã STT (`MlkitTranslatePlugin.LANG_MAP`) | Chỉ dùng khi biết trước `sourceLang` (luồng nghe trực tiếp) - dịch thủ công/dịch file luôn bỏ qua tầng này |
| 2 | Groq (LLM, `gpt-oss-120b`→`qwen3.8-27b`→`gpt-oss-20b`) | Online, quota theo key | **Đủ cả 29/29** mã (`LANG_NAMES` trong `groq-relay/index.ts`) | Tự thử `GROQ_API_KEY_2` nếu key 1 hết quota/401 trước khi rơi xuống tầng 3. Circuit breaker 2 lớp: (a) phía client (`app.js`) - 3 lỗi liên tiếp thì bỏ qua Groq 60s; (b) phía server (`groq-relay/index.ts`, best-effort theo isolate còn "ấm") - trả lỗi ngay khi cả 2 key đều đang 401/429, không bắn thêm request tới Groq. Xem trạng thái breaker server qua `?op=debug` (trường `groqCircuit`) |
| 3 | Gemini (LLM, `gemini-3.5-flash-lite`→`gemini-2.5-flash-lite`) | Online, free tier theo key | **Đủ cả 29/29** mã (dùng chung `LANG_NAMES`) | Tầng LLM dự phòng thứ 2 - đặt trước DeepL/Azure vì phủ hết ngôn ngữ như Groq. Bỏ qua hoàn toàn nếu chưa set `GEMINI_API_KEY` |
| 4 | DeepL | Online, 500.000 ký tự/tháng (free tier) | 16/29 mã (`DEEPL_TARGET_LANG_MAP` trong `app.js`) - chất lượng tốt nhất cho các cặp Âu-Âu | Bỏ qua hoàn toàn nếu chưa set `DEEPL_API_KEY` |
| 5 | Azure Translator | Online, 2.000.000 ký tự/tháng (free tier F0) | Chỉ 3 mã DeepL không có: `kz`→`kk`, `uz`, `ky` (`AZURE_TARGET_LANG_MAP` trong `app.js`) | Bỏ qua hoàn toàn nếu chưa set `AZURE_TRANSLATOR_KEY`/`AZURE_TRANSLATOR_REGION`. Cố tình KHÔNG dùng cho các mã DeepL đã hỗ trợ - tránh 2 tầng trùng việc |
| 6 | Google Translate free (endpoint không chính thức) | Online, không key nhưng hay bị 429 | Tất cả (tự nhận diện `sl=auto`) | Tự retry 3 lần (backoff 1s/2s/3s) khi 429, hết retry thì báo lỗi hẳn cho câu đó |

Kết quả dịch được cache theo `(sourceLang, targetLang, text)` trong
localStorage (tối đa 500 entry) - câu lặp lại (chào hỏi, quảng cáo lặp trong
video dài) không tốn thêm lượt gọi mạng nào ở bất kỳ tầng nào trên.

**5 mã ngôn ngữ không có tầng offline (không có trong ML Kit)**: `kz`
(Kazakh), `uz` (Uzbek), `tg` (Tajik), `ky` (Kyrgyz), `br` (Breton) - vẫn nghe
được bình thường (có model Vosk STT) nhưng dịch luôn phải qua tầng mạng ở
trên, nên phụ thuộc mạng nhiều hơn các ngôn ngữ khác. Cả 5 mã đều được Groq
(tầng 2) hỗ trợ; riêng `kz`/`uz`/`ky` có thêm Azure (tầng 4) làm lưới an
toàn quota lớn khi Groq hết quota. `tg` (Tajik) và `br` (Breton) hiện không
có dịch vụ free nào khác đã xác minh hỗ trợ đủ tốt để thêm vào chuỗi (xem
lịch sử audit) - 2 mã này rơi thẳng từ tầng 2 (Groq) xuống tầng 5 (Google
free) nếu Groq hết quota.
...
- **Danh sách "Ngôn ngữ đang nói" tự đổi nguồn theo trạng thái cloud STT**: bật lên thì dropdown
  đổi sang `WHISPER_LANGS` (100 mã ISO-639-1 mà Whisper/Groq hỗ trợ, xem `www/index.html`) thay
  vì 29 mã catalog Vosk - dùng ngay không cần tải, và bao phủ nhiều ngôn ngữ Vosk không có (vd
  Ả Rập, Quảng Đông `yue`...). Tắt cloud STT thì dropdown quay lại đúng danh sách Vosk như cũ.
  Lưu ý 1 số mã KHÁC nhau giữa 2 nguồn cho cùng ngôn ngữ vì Vosk dùng mã app tự đặt còn Whisper
  dùng đúng ISO-639-1: Kazakh Vosk=`kz` nhưng Whisper=`kk`; Kyrgyz (`ky`) và Esperanto (`eo`) có
  ở Vosk nhưng KHÔNG có ở Whisper (bị lược khỏi dropdown khi bật cloud STT cho 2 ngôn ngữ này).
  BUG đã sửa kèm theo: trước đây bật cloud STT với 1 ngôn ngữ Vosk chưa tải sẵn thì nút "Bắt đầu
  nghe" vẫn bị disable sai (do `updateSourceLangHint()` không biết cloud STT đã bypass bước
  kiểm tra model) - `isSourceLangModelReady()` giờ trả `true` ngay khi cloud STT đang bật.

## Micro ngoài (tai nghe dây/USB/Bluetooth) khi chọn "Ghi âm qua micro"

Khi "Nguồn audio" = **"Ghi âm qua micro"**, checkbox mới **"🎧 Ưu tiên micro ngoài nếu có"** xuất
hiện ngay bên dưới. Bật lên để cố định route `AudioRecord` sang thiết bị thu NGOÀI đang kết nối
(thay vì micro liền trong máy) - hữu ích khi micro máy quá xa nguồn tiếng hoặc chất lượng kém hơn
tai nghe/micro rời đang có sẵn.

- **Native**: `AudioCaptureService.selectMicInputDevice()` gọi `AudioRecord.setPreferredDevice()`
  (API 23+, chỉ đổi ĐƯỜNG THU/routing, không đổi `AudioSource` - vẫn giữ
  `MediaRecorder.AudioSource.VOICE_RECOGNITION` để hưởng AEC/NS nếu thiết bị hỗ trợ). Thiết bị
  được chọn qua `findBestExternalInputDevice()`, ưu tiên USB (`TYPE_USB_HEADSET`/`TYPE_USB_DEVICE`)
  > tai nghe dây (`TYPE_WIRED_HEADSET`/`TYPE_WIRED_HEADPHONES`) > Bluetooth SCO
  (`TYPE_BLUETOOTH_SCO`).
- **Giới hạn Bluetooth**: app KHÔNG tự gọi `AudioManager.startBluetoothSco()` - tai nghe Bluetooth
  chỉ xuất hiện trong danh sách thiết bị nếu hệ thống/app khác đã bật sẵn kênh thoại SCO. A2DP
  (chế độ nghe nhạc chất lượng cao mặc định của hầu hết tai nghe Bluetooth) không có đường micro
  về máy - đây là giới hạn phần cứng/giao thức Bluetooth, không phải thiếu sót phía app.
- **Không tìm thấy thiết bị ngoài nào đang cắm** (hoặc `setPreferredDevice()` thất bại, hiếm gặp -
  thường do rút thiết bị đúng lúc gọi): tự fallback về micro trong máy, không báo lỗi (đây là
  hành vi có chủ đích trước đây, chỉ đổi route nếu chắc chắn có gì đó tốt hơn).
- **UI**: dòng trạng thái ngay dưới checkbox tự cập nhật 2 lần - lúc **chưa** bắt đầu nghe (chỉ dò
  danh sách qua `AudioCapturePlugin.listInputDevices()`, không route gì) để user biết trước có
  nên tick hay không, rồi ghi đè bằng kết quả **thật** (sự kiện `micDeviceInfo`) ngay sau khi
  native đã chọn xong thiết bị lúc bắt đầu nghe. Lựa chọn được lưu vào `localStorage`
  (`preferExternalMic`) và phục hồi đúng qua `START_STICKY` nếu service tự khởi động lại sau khi
  bị OS kill (giống `smartPause`/`cloudStt`).

## Ghi chú sửa lỗi gần đây

- **Test coverage cho `AdBlockList.kt`/`normalizeUrl()` trong
  `BrowserTabManager.kt`**: 2 phần logic thuần này (chặn quảng cáo/tracker
  theo domain ở "Web mini", và quyết định ô địa chỉ mở URL hay tìm kiếm) chưa
  từng có test tự động. Đã port sang `tests/adblock/` (14 test - danh sách
  domain đọc THẲNG từ `native/AdBlockList.kt` như cách `tests/model_catalog/`
  đọc CATALOG, tránh lệch pha) và `tests/browser_url/` (18 test - trọng tâm
  là ký tự "vô hình" dính theo khi copy link từ app khác, đúng loại bug đã
  từng gặp thật). Xem README riêng ở từng thư mục.
- **Lỗi có sẵn phát hiện khi làm việc trên**: `tests/model_catalog/catalog_parser.py`
  trỏ nhầm sang `native/ModelManagerPlugin.kt`, trong khi `CATALOG` thật đã
  được tách riêng sang `native/ModelCatalog.kt` từ trước - khiến
  `parse_catalog()` luôn raise lỗi "Không tìm thấy 'val CATALOG:'", nghĩa là
  toàn bộ bộ test `tests/model_catalog/` (kiểm URL model có hợp lệ không,
  đúng bug F đã ghi ở dưới) đã fail âm thầm mọi lần CI chạy trước đây mà
  không ai để ý (không phải do lỗi mới của bản zip vừa sửa cấu trúc thư mục -
  đường dẫn sai này tồn tại độc lập, kể cả với cấu trúc phẳng cũ). Đã sửa lại
  đường dẫn, verify lại parse đúng 29 entries.
...
- **Test coverage cho `BrowserActivity.kt`/`CaptionRelay.kt`**: 2 file này
  (`BrowserActivity` 40KB) trước đó không có test tự động nào, khác các file
  đã có port (`VadDetector`/dịch/`model_catalog`). Đã port riêng phần TÍNH
  TOÁN THUẦN của `applyCaptionFrame()` (quy đổi opacity→alpha, fontSize→sp,
  widthPercent/heightPercent→px theo `coerceIn`) sang
  `tests/caption_frame/caption_frame.py` + 23 test hồi quy, tập trung đúng
  chỗ từng có bug thật (khung phụ đề hẹp bất thường ở "Web mini") — xem
  `tests/caption_frame/README.md`. Phần đụng Android View thật (quản lý tab,
  AdBlockList cho WebView, add đúng vị trí `captionOverlay`...) vẫn ngoài
  phạm vi port này, cần build thật để test.
- **Test coverage cho tính năng Whisper Cloud (Groq) trong
  `AudioCaptureService.kt`**: file lớn nhất repo (47KB) trước đó cũng chưa
  có test tự động. Đã port `shortsToBytesLE()`/`buildWavFile()` (đóng gói
  WAV) và state machine cắt đoạn trong `startCloudSttReadLoop()`
  (`CLOUD_SEGMENT_PAUSE_MS`/`MAX_MS`/`MIN_BYTES`) sang
  `tests/cloud_stt_segment/` + 23 test hồi quy — trước đây tính năng này
  chỉ được xác nhận bằng đếm ngoặc cân bằng, chưa từng chạy thử hành vi
  thật. Xem `tests/cloud_stt_segment/README.md`.
- **Test coverage cho "Ngắt câu thông minh" nhánh Vosk**: phần logic thuần
  còn lại của `AudioCaptureService`/`SttReadLoop.kt` chưa có test (nhánh
  `startVoskAndReadLoop()`, khác hẳn state machine Cloud STT ở trên). Đã port
  state machine `lastVoiceAt`/`pauseHandled` quyết định khi nào gọi
  `forcePauseFinal()` (`SMART_PAUSE_MS`) sang `tests/vosk_smart_pause/` + 10
  test hồi quy (chạm ngưỡng, chặn gọi lặp, tiếng nói ngắn xen giữa khoảng
  nghỉ dời lại đúng mốc tính, nhiều câu liên tiếp). Xem
  `tests/vosk_smart_pause/README.md`.
...
> **Đã sửa lỗi cấu trúc thư mục trong zip (2026-09-02):** các bản zip
> `atun-translator-part*.zip` trước đây đóng gói file `native/*.kt` và
> `www/*` (html/js/css) ở dạng PHẲNG (không có thư mục cha `native/`/`www/`),
> trong khi `.github/workflows/android-build.yml` lại cần đúng 2 thư mục này
> (`cp native/*.kt ...` và Capacitor `webDir: 'www'`) - giải nén thẳng theo
> hướng dẫn bên dưới trước đây sẽ khiến bước copy Kotlin lỗi "no such file"
> hoặc Capacitor không tìm thấy web assets. Từ bản zip hiện tại, `part1`
> (native) đã chứa sẵn thư mục `native/` và `part2` (www) đã chứa sẵn thư mục
> `www/` bên trong - giải nén theo đúng hướng dẫn dưới đây giờ ra đúng cấu
> trúc `native/*.kt` + `www/*` như workflow cần, không cần sửa gì thêm.
